...
public class JPAUsuarioDAO extends JPAGenericDAO<Usuario, Integer> implements UsuariosDAO{
	...
	public Usuario validarU(String correo, String password) {
		...
		String consulta="SELECT * FROM Usuario u WHERE u.correo='"+correo+"' AND u.password='"+password+"'";
		try {
			Usuario respuesta=(Usuario) em.createQuery(consulta).getSingleResult();	
			if(respuesta != null) {
	...
	public List<Usuario> findU(int id) {
		...
		String consulta =("SELECT * FROM Usuario u WHERE u.empresa.id = "+id+" AND u.rol = 'U'");
		try {
				respuesta=(List<Usuario>)em.createQuery(consulta).getResultList();				
		}catch(Exception e) {
